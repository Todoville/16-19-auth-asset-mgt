...
export default (request, response, next) => {
  ...
  const token = request.headers.authorization.split('Bearer')[1]; //no space after bearer???
  ...
  return jwtVerify(token, process.env.SECRET_KEY)
    ...
    .then((account) => {
      if (!account) return next(new HttpErrors(400, 'BEARER AUTH - no account found my duders'));
